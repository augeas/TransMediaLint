...
class SearchSpider(scrapy.Spider):
    ...
    async def parse_results(self, response):
        ...
        title_headers = response.css(
            'div.compressed-mobile>div.post>a.block>h3.title-sm')

        titles = [
            ' '.join(t.xpath('descendant-or-self::*/text()').extract()).strip()
            for t in title_headers
        ]

        published_xp = (
            'div.compressed-mobile>div.post>'
            + 'a.block>div.post-meta>div.post-date'
        )
        published = list(
            map(datetime.isoformat, map(parser.parse, response.css(
